// Stages Phase 8 edge fns to %LOCALAPPDATA%\Temp\sb-deploy-phase8\
// per D-170 (Supabase CLI needs a clean config.toml without legacy keys).
//
// After staging, deploy each from the staged workdir, e.g.:
//   npx supabase functions deploy mastery-recompute \
...
const root = path.join(process.env.LOCALAPPDATA, "Temp", "sb-deploy-phase8");
