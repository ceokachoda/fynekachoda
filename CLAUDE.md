...
- **Phase 7 (Graded Examinations) ✅ code-complete — 2026-05-19.** CP1–CP14 + CP9b (`exam-attempt-result` per D-175 + `exam-admin-mutate` per D-177) + CP15 (tests + advisor sweep) all green; manual QA pending. 2 new migrations applied (`exams`, `exams_rls`), 9 edge fns deployed (`server-time` with `--no-verify-jwt`, `exam-start`, `exam-tab-switch`, `exam-submit`, `exam-release-results`, `exam-regrade`, `offline-score-upsert`, `exam-attempt-result`, `exam-admin-mutate`). Mobile: 4 top-level Stack screens (`app/exam/[id].tsx` locked-down 5-stage attempt UI per D-169; `app/exam-builder/[examId].tsx` teacher builder; `app/exam-results/[examId].tsx` roster + regrade; `app/offline-scores.tsx` paper-test entry), new `(teacher)/exams.tsx` tab, new `TabSwitchBanner` component, 9 feature hooks (`useExamStart`/`useExamSubmit`/`useExamAttemptResult`/`useExamAutoSave`/`useExamTabSwitchLogger`/`useServerTimeOffset`/`useTeacherExams`/`useTeacherExamBuilder`/`useStudentExams`/`useExamResultsBoard`/`useOfflineScores`/`useBatchSubjects`/`useTeacherBatches`), new `lib/edge-fn.ts` typed POST helper that exposes HTTP status (needed for 423 LOCKED + 409 replay). Student dashboard surfaces an Exams section above Weak Topics. Admin: `/exams` (status badges + filters + force-release + delete + CSV export) and `/offline-scores` (per-batch + CSV + delete) — all mutations route through `exam-admin-mutate` (D-172). Tests: 26/26 unit (`pnpm test:exam` — grading + sanitisation + regrade overrides + server-time helpers), 12/12 RLS (`pnpm smoke:exam-rls` including T10 stringify-and-grep proving zero `correct_option_id` + zero `is_correct` leak in `exam-start`), 26/26 edge-fn HTTP (`pnpm smoke:exam-fns` covering all 9 fns + audit row verification + replay protection + 3 stacked regrade actions + offline-score insert-vs-update), 53/53 mobile jest, typecheck + lint green; advisor sweep clean (0 ERRORs, all WARNs are pre-existing-class `multiple_permissive_policies` from intentional role segmentation, 5 INFOs are brand-new-index + unindexed-FK on rarely-traversed paths). Manual test plan at `docs/phases/phase-7-manual-tests.md` + fixture script `pnpm seed:exam-manual-test` (with `--reset` from day one). Ledger in `docs/phases/phase-7.md §15`. Four hard-learned lessons memorialised: **D-179** (regrade MUST do full per-attempt recompute — stacked regrades drift with delta math; fix via `snapshot.question.regrade_override`), **D-180** (`exam_attempts.question_snapshot` is a self-contained server-side grading dossier with full content + correct_option_id, NOT just an order hint — Phase 6 quiz semantics differ), **D-181** (`server-time` edge fn deploys with `--no-verify-jwt` because it's called during token refresh windows), **D-182** (`exam-tab-switch` is fire-and-forget on the client AND silently no-ops on already-submitted attempts on the server to prevent the AppState-fires-after-submit race from surfacing as a spurious error toast). Phase 8 carry-overs: real `mastery-recompute` (stub TODO in `exam-submit`), consolidate teacher Quizzes + Exams tabs (bar is now 8 entries — crowded on narrow phones), replace student `(student)/classes.tsx` Phase-0 placeholder with `useStudentExams` shape, component-level jest for locked-down attempt UI, two-device tab-switch debounce, Redmi 8A cold-start on `app/exam/[id].tsx` (still hardware-blocked from Phase 5). Work sits uncommitted on `phase-4` branch pending the consolidated PR. 4 new migrations applied (`question_bank`, `quizzes`, `quiz_rls`, `storage_exam_images`), 5 edge fns deployed (`quiz-start`, `quiz-submit`, `quiz-attempt-result`, `quiz-image-presign`, `quiz-admin-mutate`), 1 new private Storage bucket (`exam-images` ≤5 MB jpeg/png/webp). Mobile: rebuilt student `app/quiz/[id].tsx` as a 4-stage state machine (intro → attempt → result → solution) at top-level per D-169, with `MathText` rendering KaTeX via WebView only when math is detected (D-178), debounced auto-save to `quiz_answers` via PostgREST (RLS-permitted for in-flight attempts), server-anchored countdown timer, navigation grid, flag toggle, retake. Mobile teacher: new `(teacher)/quizzes.tsx` tab + top-level `app/quiz-builder/[quizId].tsx` with cascading topic picker + inline question editor + "Add from bank" sheet. Mobile student dashboard wires `useWeakTopics` proxy (avg <70% per topic) and library shows quizzes at topic level. Admin: `/quizzes` and `/questions` pages with CSV export + filters + publish toggle + archive + delete — all mutations routed through `quiz-admin-mutate` so `audit_log` captures before/after (D-172). Tests: 27/27 helper unit smokes (`pnpm test:quiz`), 12/12 RLS scenarios (`pnpm smoke:quiz-rls`) including T12 stringify-and-grep proving zero `"is_correct"` leak in `quiz-start` response, 26/26 edge-fn HTTP assertions (`pnpm smoke:quiz-fns`), 53/53 mobile jest, typecheck + lint green; advisor sweep clean (only Phase-1 `auth_leaked_password_protection` backlog WARN remains). Four hard-learned lessons memorialised: D-175 (`quiz-attempt-result` re-fetch fn — spec calls for re-openable solution view + image URLs expire), D-176 (PostgREST `Prefer: return=representation` evaluates BOTH WITH CHECK and USING on the inserted row — cross-scope inserts in fixtures must use service role), D-177 (one admin-mutate edge fn per resource family — reaffirms D-151; `quiz-admin-mutate` carries 4 ops), D-178 (`MathText` mounts a WebView only when KaTeX delimiters are detected — plain text renders to native `<Text>` for low-end perf). Phase 7+ carry-overs: local KaTeX bundle, mastery-recompute wiring, component-level jest, image-picker UX in quiz-builder, Redmi 8A cold-start (still hardware-blocked from Phase 5). Manual test plan at `docs/phases/phase-6-manual-tests.md` + fresh fixture script `pnpm seed:quiz-manual-test` (with `--reset` flag from day one — closes Phase-5 seed-hygiene carry-over). Ledger in `docs/phases/phase-6.md §14`. Work sits uncommitted on `phase-4` branch pending the consolidated PR.
