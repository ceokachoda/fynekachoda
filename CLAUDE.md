...
- **Phase 7 (Graded Examinations) 🟡 CODE-COMPLETE, MANUAL QA PENDING — 2026-05-19.** ⚠ NOT yet manually verified end-to-end on a real device or in a real browser. CP1–CP16 are code-complete and all 118 automated assertions pass (see ledger in `docs/phases/phase-7.md §15`), but bugs in the locked-down attempt UI, AppState/tab-switch wiring, server-anchored timer behaviour, regrade modal flows, offline-scores roster rendering, and admin force-release / delete buttons are EXPECTED and have not been ruled out. Server-time enforcement bugs can let students cheat and regrade bugs can cause grade disputes — these are high-stakes surfaces. Manual test plan at `docs/phases/phase-7-manual-tests.md` (1,227 lines, click-by-click with explicit "Report:" lines per step). User is walking this in a dedicated QA session — patches will land before the ACCEPTED line gets filled in. **Do NOT mark Phase 7 as "done" until the manual QA is signed off.** 2 new migrations applied (`exams`, `exams_rls`), 9 edge fns deployed (`server-time` with `--no-verify-jwt`, `exam-start`, `exam-tab-switch`, `exam-submit`, `exam-release-results`, `exam-regrade`, `offline-score-upsert`, `exam-attempt-result`, `exam-admin-mutate`). Mobile: 4 top-level Stack screens per D-169 (`app/exam/[id].tsx` locked-down 5-stage attempt UI; `app/exam-builder/[examId].tsx`; `app/exam-results/[examId].tsx` with regrade modal; `app/offline-scores.tsx`), new `(teacher)/exams.tsx` tab (teacher bar now 8 entries — crowded), new `TabSwitchBanner`, 13 feature hooks, new `lib/edge-fn.ts` typed POST helper. Student dashboard surfaces Exams section above Weak Topics. Admin: `/exams` + `/offline-scores` pages — all mutations through `exam-admin-mutate` (D-172). Tests (automated only): 26/26 unit (`pnpm test:exam`), 12/12 RLS (`pnpm smoke:exam-rls` incl. T10 stringify-and-grep proving zero `correct_option_id` + zero `is_correct` leak in `exam-start`), 26/26 edge-fn HTTP (`pnpm smoke:exam-fns` incl. 3 stacked regrade actions), 53/53 mobile jest, typecheck + lint green; advisor sweep clean. Decisions D-179..D-182: full-recompute regrade via `snapshot.regrade_override`; `question_snapshot` is self-contained grading dossier; `server-time` deploys `--no-verify-jwt`; `exam-tab-switch` fire-and-forget + silent-on-submitted. Seed: `pnpm seed:exam-manual-test --reset`. Phase 8 carry-overs: real `mastery-recompute`, consolidate Quizzes+Exams tabs, swap student `(student)/classes.tsx` placeholder, component-level jest, two-device tab-switch debounce, Redmi 8A cold-start. Work sits uncommitted on `phase-4` branch.
