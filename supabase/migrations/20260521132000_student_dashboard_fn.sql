-- Phase 8 CP5: student_dashboard(p_student) — single-round-trip JSON for the student home.
-- SECURITY DEFINER (composes RLS-protected tables) but guards that the caller may only
-- read their own dashboard (or is an admin). next_card implements the 7 priorities of
-- spec/student-dashboard.md §4.1 as a COALESCE cascade (first match wins). All "today"
-- date math is IST (D-074). Every slice coalesces so new/empty students render zeros.
...
begin
  ...
  return jsonb_build_object(
    'next_card', coalesce(
      -- P1: a live class is live for the student's batch
      ...
      -- P2: an exam is within its window and the student has not submitted
      ...
      -- P3: next scheduled session within 30 minutes
      (select jsonb_build_object('type','upcoming_session','priority',3,'session_id',s.id,
       ...
       order by s.scheduled_start asc limit 1),
      -- P4: pending attendance for a session happening right now
      (select jsonb_build_object('type','attendance','priority',4,'session_id',s.id,
       ...
       order by s.scheduled_start asc limit 1),
      -- P5: most-recent unwatched recording in the student's scope
      ...
      -- P6: weakest topic (mastery<50, >=2 attempts) that has a published quiz
...
revoke all on function public.student_dashboard(uuid) from public, anon;
grant execute on function public.student_dashboard(uuid) to authenticated, service_role;
