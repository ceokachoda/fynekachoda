...
function statusBadge(r: ExamRow) {
  ...
  if (!r.is_published) return { label: "Draft", variant: "default" as const };
  if (r.results_released_at) return { label: "Released", variant: "success" as const };
  if (now < start) return { label: "Scheduled", variant: "secondary" as const };
  if (now < end) return { label: "Live", variant: "destructive" as const };
      ...
      >
        {rows.length === 0 ? (
          <EmptyState
            ...
            description="Adjust your filters or schedule a new exam."
          />
        ) : (
                ...
                return (
                  <tr key={r.id} className="hover:bg-muted/50 transition-colors">
                    ...
                    <td className="px-3 py-3 tabular-nums text-foreground/90">
                      ...
                      <div className="text-[11px] text-muted-foreground mt-0.5">{r.duration_min} min · {r.result_release}</div>
                    </td>
        ...
        >
          ...
          >
            ...
            <p className="mt-2 text-sm text-muted-foreground leading-relaxed">
              This permanently removes <strong className="text-foreground font-medium">{confirmDelete.title}</strong> and
