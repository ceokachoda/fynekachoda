// Phase 4 (Web) — pure chat-replay math (no React imports so Vitest can import
// it without jsdom). Mirrors mobile features/live/chat-replay.ts.
//
...
// No separate tick rate is needed — a faster speed advances currentTime
// faster, which reveals messages faster, exactly as the spec wants.

export function computeReplayOffsetSec(
  postedAt: string,
  startedAt: string,
): number {
...
export function withReplayOffsets<T extends { posted_at: string }>(
  ...
  return messages
    .map((m) => ({
      ...m,
      offsetSec: computeReplayOffsetSec(m.posted_at, startedAt),
    }))
...
export function countBetween<T extends { offsetSec: number }>(
  ...
  return items.filter(
    (m) => m.offsetSec > fromSec + 0.001 && m.offsetSec <= toSec + 0.001,
  ).length;
}
