...
export function ScheduleLiveSheet({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(
        isNetworkError(err) ? NETWORK_ERROR_MESSAGE : "Couldn't schedule the live class.",
      );
      setSubmitting(false);
    }
  };
        ...
        <View className="bg-white rounded-t-[28px] px-6 pt-5 pb-8">
          <View className="flex-row items-center justify-between mb-1">
            <View className="flex-row items-center">
              <Radio size={20} color="#dc2626" style={{ marginRight: 8 }} />
          ...
          {showBatchPicker ? (
            ...
            >
              {batches.map((b) => (
                ...
                >
                  <Text className="text-sm font-semibold text-slate-900">{b.batch_name}</Text>
            ...
            {DURATION_OPTIONS_MIN.map((d) => (
              <TouchableOpacity
                ...
                className={`px-3 py-2 rounded-full border mr-2 ${
                  d === durationMin ? "bg-blue-600 border-blue-600" : "bg-white border-slate-200"
                }`}
              >
                <Text
          ...
          {error ? <Text className="text-red-600 text-xs mt-3">{error}</Text> : null}
          ...
          >
            {submitting ? (
              ...
              <Text className="text-white font-bold text-sm">Set up live class</Text>
            )}
          </TouchableOpacity>
        </View>
      </View>
